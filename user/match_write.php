<?php
	...
	$pickTeam = $row['at_idx']."||".$row['am_team_name'];
	$teamOn = true;
	if($w == "u"){
		$teamOn = false; //이 글을 등록할 때 원래 있던 팀을 마이페이지에서 지운 경우 대비
		for($t=1; $t<=3; $t++){
			if($member['mb_user_team'.$t] == $row['at_idx']){
				$teamOn = true;
				break;
			}
		}
	...
	<form name="wr_frm" method="post" autocomplete="off" enctype="multipart/form-data">
		...
		<ul class="regi_ul">
			<li class="regi_li">
				...
				<div class="regi_td">
					<select name="at_idx" id="at_idx" class="regi_ipt req_ipt regi_select regi_select2" onChange="fnValueCount();">
						<option value="">
							<?php if(!$teamOn){?>기존에 있던 팀을 삭제하셨어요!<?php }else{?>팀을 선택하세요.<?php }?>
						</option>
						
						<?php for($t=1; $t<=3; $t++){?>
							<?php if($member['mb_user_team'.$t]){ ?>
							<option value="<?php echo $member['mb_user_team'.$t]."||".getTeamName($member['mb_user_team'.$t])?>" <?php if($member['mb_user_team'.$t] == $row['at_idx']){ echo "selected"; }?>>
		...
		<div class="fix_btn_box">
			<button type="button" class="fix_btn" id="submit_button" onClick="register();">
				<?php if($w == "u"){ echo "매치 수정"; }else{ echo "매치 등록";  }?>
			</button>
		</div>
