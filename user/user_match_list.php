...
<div class="my_match_res">
	<div class="mmr_menu">
		<p>
			<input type="radio" name="menu" id="menu_1" value="1" checked onChange="filterSubmit();">
			<label for="menu_1">매치 신청 / 등록</label>
		</p>
		<p>
			<input type="radio" name="menu" id="menu_2" value="2" onChange="filterSubmit();">
			<label for="menu_2">매치 완료</label>
		</p>
	</div>
	<div class="user_filter_box2">
		<select name="s_team" id="s_team" class="simple_ipt2" onChange="filterSubmit();">
		...
		<select name="s_type" id="s_type" class="simple_ipt2" onChange="filterSubmit();">
			<option value="">매치 전체</option>
			<option value="1">매치 등록</option>
			<option value="2">매치 신청</option>
			<option value="3">리매치 요청</option> <!-- 받은거=신청 -->
			<option value="4">리매치 신청</option> <!-- 보낸거=등록 -->
			<option value="5">매치 확정</option>
		</select>
	</div>
...
function filterSubmit(v){
	const menu = $("input[name=menu]:checked").val();
	const team = $("#s_team").val();
	let type = $("#s_type").val();
	...
	$.ajax({
		type: "POST",
		url: "<?php echo G5_URL?>/inc/match_res_list.php",
		data: {state_type:menu, team_idx:team, match_type:type, limitVal:limitVal}, 
		cache: false,
		async: false,
