...
for($i=0; $i<count($team_name); $i++){
	...
	if($i == 0){
		$sql3 = " update {$tb} set mb_user_team1 = '{$at_idx}' where mb_no = '{$mb_idx}' ";
		sql_query($sql3);
	}else if($i == 1){
		$sql3 = " update {$tb} set mb_user_team2 = '{$at_idx}' where mb_no = '{$mb_idx}' ";
		sql_query($sql3);
	}else if($i == 2){
		$sql3 = " update {$tb} set mb_user_team3 = '{$at_idx}' where mb_no = '{$mb_idx}' ";
		sql_query($sql3);
	}
}
