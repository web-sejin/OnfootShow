...
$add_query = "";
if($s_txt != ""){ 
	$add_query .= " and (an_subject like '%{$s_txt}%' or an_content like '%{$s_txt}%') "; 
	$qstr .= "&s_txt=".$s_txt;
}
...
$sql_order = " order by an_datetime desc ";

$sql = " select count(distinct an_idx) as cnt {$sql_common} {$sql_search} {$sql_order} ";
$row = sql_fetch($sql);
$total_count = $row['cnt'];
...
<section id="">
	<form name="fsearch" id="fsearch" class="local_sch01 local_sch" method="get">
		...
		<div class="filter_box">
			...
			<div class="filter_list">
				<input type="text" name="s_txt" value="<?php echo $s_txt ?>" id="s_txt" class="frm_input" placeholder="제목 또는 내용을 입력해주세요.">
	...
	<form name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
		...
		<input type="hidden" name="prim" value="an_idx">
		...
		<div class="tbl_head01 tbl_wrap">
			...
			<tr class="<?php echo $bg; ?>">
				<td><input type="checkbox" name="chk_id[]" value="<?php echo $row['an_idx']?>"></td>
				<td><?php echo ($page-1)*$rows+$i+1?></td>
				<td class="txtleft"><?php echo $row['an_subject']?></td>				
				<td><?php echo $row['an_datetime']?></td>				
				<td class="td_mng td_mng_m">
					<a href="a_notice_write.php?w=u&an_idx=<?php echo $row['an_idx']?>&<?php echo $qstr?>" class="btn btn_03">수정</a>
					<button type="button" class="btn btn_02" onClick="fnDeleteType('<?php echo $tb?>', 'an_idx', '<?php echo $row['an_idx']?>');">삭제</button>
				</td>
			</tr>
			<?php
			}
		...
		<div class="btn_fixed_top">
			...
			<a href="./a_notice_write.php" id="bo_add" class="btn_01 btn">등록</a>
			<?php } ?>
		</div>
