...
$add_query = "";
if($s_txt != ""){ 
	$add_query .= " and A.aq_subject like '%{$s_txt}%' "; 
	$qstr .= "&s_txt=".$s_txt;
}
//if($s_start != ""){ $add_query .= " and at_start_date >= '{$s_start}' "; }
//if($s_end != ""){ $add_query .= " and at_end_date <= '{$s_end}' "; }

$sql_common = " from {$tb} A, g5_member B ";
$sql_search = " where A.mb_idx = B.mb_no {$add_query} ";
$sql_order = " order by A.aq_datetime desc ";

$sql = " select count(distinct A.aq_idx) as cnt {$sql_common} {$sql_search} {$sql_order} ";
$row = sql_fetch($sql);
$total_count = $row['cnt'];
...
<section id="">
	<form name="fsearch" id="fsearch" class="local_sch01 local_sch" method="get">
		...
		<div class="filter_box">
			...
			<div class="filter_list">
				<input type="text" name="s_txt" value="<?php echo $s_txt ?>" id="s_txt" class="frm_input" placeholder="아이디 or 제목을 입력해주세요.">
	...
	<form name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
		...
		<input type="hidden" name="prim" value="aq_idx">
		...
		<div class="tbl_head01 tbl_wrap">
			...
			<tr class="<?php echo $bg; ?>">
				<td><input type="checkbox" name="chk_id[]" value="<?php echo $row['aq_idx']?>"></td>
				<td><?php echo ($page-1)*$rows+$i+1?></td>
				<td>
					<?php echo $row['mb_id']?><br>
					(<?php echo $row['mb_name']?>)
				</td>	
				<td><?php echo $row['aq_subject']?></td>	
				<td><?php echo $row['aq_datetime']?></td>				
				<td><?php if($row['aq_answer']){ echo "완료";  }else{ echo "대기";  }?></td>	
				<td class="td_mng td_mng_m">
					<a href="a_qna_write.php?w=u&aq_idx=<?php echo $row['aq_idx']?>&<?php echo $qstr?>" class="btn btn_03">수정</a>
					<button type="button" class="btn btn_02" onClick="fnDeleteType('<?php echo $tb?>', 'aq_idx', '<?php echo $row['aq_idx']?>');">삭제</button>
				</td>
			</tr>
			<?php
			}
