...
//지역 값
function getSido($idx){
	$sql = " select * from rb_sido where sd_idx = '{$idx}' ";
	$row = sql_fetch($sql);

	return $row['sd_name'];
}

function getSigugun($idx){
	if($idx == "all"){
		return "전체";
	}else{
		$sql = " select * from rb_sigungu where si_idx = '{$idx}' ";
		$row = sql_fetch($sql);
...
function getFirstStadium($mb_no){
	$sql = " select * from a_stadium where mb_idx = '{$mb_no}' and as_delete_st = 1 order by as_idx asc limit 1 ";
	$row = sql_fetch($sql);

	return $row['as_idx'];
}

function getFirstPrice($mb_no){
	$sql = " select * from a_stadium where mb_idx = '{$mb_no}' and as_delete_st = 1 order by as_idx asc limit 1 ";
	$row = sql_fetch($sql);
...
function getStadiumPrice($as_idx){
	$sql = " select * from a_stadium where as_idx = '{$as_idx}' ";
	$row = sql_fetch($sql);

	return $row['as_price'];
}

function getOtherTeam($idx, $col){
	$result = "";
	$sql = " select * from a_other_team where other_idx = '{$idx}' ";
	$row = sql_fetch($sql);	
	
	if($col == "other_sort"){
		if($row['other_sort'] == 1){ $result = "일반";  }else if($row['other_sort'] == 2){ $result = "고정";  }
	}else{
		$result = $row[$col];
	}
...
//앱 회원 자동로그인
function fnAutoLogin($idx, $token){
...
function fnTokenUpdate($idx, $token){
	$sql = " update g5_member set
					mb_token = NULL
					where 1
						and mb_no != '{$idx}'
...
function getTeamName($idx){
	$sql = " select * from a_team where at_idx = '{$idx}' ";
	$row = sql_fetch($sql);

	return $row['at_team_name'];
}

function getFutsalStadiumName($idx){
	$sql = " select * from a_stadium A, g5_member B where A.mb_idx = B.mb_no and A.as_idx = '{$idx}' ";
	$row = sql_fetch($sql);

	return $row['mb_fs_name'];
}

function getStadiumName($idx){
	$sql = " select * from a_stadium where as_idx = '{$idx}' ";
	$row = sql_fetch($sql);

	return $row['as_name'];
}

function getLocalName($sd_idx, $si_idx, $do_idx){
	$result = "";
...
function reqCnt($idx){
	$sql = " select count(*) cnt from a_match_req where am_idx = '{$idx}' ";
	$row = sql_fetch($sql);

	return $row['cnt'];
}

function getMemberinfo($mb_no, $col){
